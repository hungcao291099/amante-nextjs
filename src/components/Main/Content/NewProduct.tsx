...
export default () => {
    const [NewCateApi, setNewCaterApi] = useState<BestProductCate>()
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await api({
                    url: `/shop/product/main/new/list?category1_cd=${category_cd}`,
                    method: "GET",
                });
    ...
    if (NewCateApi?.success && ProductListApi?.success) {
        return (
            <div className=" rounded-lg bg-white p-2 flex flex-col gap-5 relative">
                <div className=" flex justify-between items-center">
                    <h2 className=" font-bold text-2xl">What's New <span className=" text-sm text-gray-400">새로 들어온 상품</span></h2>
                ...
                <div className=" flex gap-3 w-full hover:cursor-pointer overflow-x-auto no-scrollbar ">
                    {NewCateApi?.data && NewCateApi.data.slice().reverse().map(cate => (
